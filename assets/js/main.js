(function ($) {
  ...
  $(".th-slider").each(function () {
    ...
    function updateNavBackground(swiper) {
      ...
      var prevSlide = slides.eq(
        (activeIndex - 1 + slides.length) % slides.length
      );

      // Get panorama images from next and previous slides
      var nextImage = nextSlide
        .find(".panorama-container")
        .data("panorama-src");
      var prevImage = prevSlide
        .find(".panorama-container")
        .data("panorama-src");

      // Apply images to navigation buttons
      if (nextImage)
        nextArrow.css("background-image", "url(" + nextImage + ")");
      if (prevImage)
        prevArrow.css("background-image", "url(" + prevImage + ")");
    }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const productHotspotDots = document.querySelectorAll(
      ".product-hotspot-dot"
    );
    ...
    hotspotDots.forEach((dot, index) => {
      dot.addEventListener("mouseenter", function () {
        ...
        const productDot = document.querySelector(
          `.product-hotspot-dot${index + 1}`
        );
    ...
    document.addEventListener("click", function (event) {
      if (
        !event.target.closest(".hotspot-dot") &&
        !event.target.closest(".product-hotspot-dot")
      ) {
        productHotspotDots.forEach((dot) => dot.classList.remove("show"));
      }
    });
  ...
  VanillaTilt.init(document.querySelectorAll(".cta"), {
    max: 10, // how much the card tilts
